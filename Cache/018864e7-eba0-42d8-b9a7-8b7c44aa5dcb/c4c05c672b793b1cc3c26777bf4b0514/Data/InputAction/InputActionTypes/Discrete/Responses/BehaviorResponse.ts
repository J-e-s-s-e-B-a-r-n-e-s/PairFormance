        ...
        const triggerFunction = function fetchTriggerFunction() {
            ...
            switch (config.behaviorResponseType) {
                case BehaviorResponseType.GlobalCustomTrigger:
                    ...
                    const behaviorSystem = (global as any)["behaviorSystem"];
                    return () => behaviorSystem?.sendCustomTrigger(config.customTriggerName);

                case BehaviorResponseType.ManualTrigger:
                    if (!config.behaviorScript) {
                        context.warn("Behavior script is not provided for manual trigger.");
                        return null;
                    }
                    return () => {
                        ...
                        if (typeof triggerMethod === "function") {
                            ...
                            context.warn("Behavior script does not have a trigger method. Skipping execution.");
                        }
                    };

                default:
                    assertNever(config.behaviorResponseType, "BehaviorResponseType");
            }
        }();
